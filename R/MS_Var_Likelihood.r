...
MS_Var_Likelihood<-function(params){
k<-2
...
Cond_mean=array(0,dim=c(nr,k*n_dep));
...
for (i in 0:(k-1)){
    Cond_mean[,(((i*n_dep)+1):((i*n_dep)+n_dep))]=Indep_S_param[,(((i*n_dep)+1):((i*n_dep)+n_dep))]; # Conditional Mean for each state
    e[,(((i*n_dep)+1):((i*n_dep)+n_dep))]=dep-Cond_mean[,(((i*n_dep)+1):((i*n_dep)+n_dep))]; # Error for each state
} 
...
n[p,]<-eta
    ...
    E=array(0,dim=c(nr,k));
    f=array(0,dim=c(nr,1));
    
    # Setting up first probs of E
    
    E[1,]=array(1/k,dim=c(k,1));

   
 for (i in 2:nr){
    Eadj<-matrix(E[i-1,],k,1)
    AdjDen<-Param_TransProbs;
    densum<-array(1,dim=c(1,k))
    nadj<-matrix(n[i],k,1)
    for (r in 1:k){
    AdjDen[,r]<-Param_TransProbs[,r]*Eadj[r]
}
   for (r in 1:k){
    densum[r]<-sum(AdjDen[r,])
        ...
        E[i,]=(densum*n[i,])/f[i];   # MS Filter equation for probabilities
